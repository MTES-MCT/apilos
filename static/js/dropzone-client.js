const DROPZONE_SELECTOR = "[data-dropzone-url]";
const DROPZONE_WORDINGS = {
  dictDefaultMessage: "Cliquez dans la zone ou déposez-y vos fichiers",
  dictFallbackMessage:
    "Votre navigateur ne supporte pas la fonction drag'n'drop. Nous vous conseillons de changer de navigateur",
  dictFallbackText: "",
  ...
  dictResponseError:
    "Le serveur a retourné une erreur, si le problème persiste, nous vous prions de contacter votre administration référente",
...
function dropzoneUrl(el) {
  return el.dataset.dropzoneUrl;
}

function initDropzone() {
  const dropzoneEls = document.querySelectorAll(DROPZONE_SELECTOR);
  for (let dropzoneEl of dropzoneEls) {
    const url = dropzoneUrl(dropzoneEl);
    const dropzone = new Dropzone(dropzoneEl.parentNode, {
      url,
      ...DROPZONE_WORDINGS,
    });
  }
...
/**
OBSOLÈTE
Une refactorisation en cours des champs d'upload vise à déprécier l'approche ci-dessous.
Elle est encore utilisée mais l'approche ci-dessus est préférable.
*/
...
function init_dropzone_from_file(form_id, accepted_files) {
  ...
  if (accepted_files == undefined) {
    ...
    accepted_files =
      accepted_files + ",application/vnd.oasis.opendocument.presentation";
    accepted_files =
      accepted_files + ",application/vnd.oasis.opendocument.spreadsheet";
    accepted_files =
      accepted_files + ",application/vnd.oasis.opendocument.text";
  }
    ...
    init: function () {
      this.on("removedfile", function (file) {
        ...
        if (document.getElementById(form_id).value)
          files = JSON.parse(document.getElementById(form_id).value);
      ...
      this.on("success", function (file, response) {
        ...
        if (document.querySelector('img[alt="' + filename + '"]') != null) {
          file.thumbnail = document.querySelector(
            'img[alt="' + filename + '"]',
          ).src;
        }
