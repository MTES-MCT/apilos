...
def deduplicate_prgramme(apps, schema_editor):
        ...
        if (
            ...
            for field in [
                ...
                if len(values := {getattr(p, field) or "" for p in programmes}) > 1:
                    ...
                    print(getattr(programmes[0], field))
            # get first not none
            for field in [
                "ville",
                "code_insee_commune",
                ...
                setattr(
                    ...
                    field,
                    next(
                        (getattr(p, field) for p in programmes if getattr(p, field)),
                        None,
                    ),
                )
            # sum values
            for field in [
                ...
                setattr(
                    ...
                    field,
                    sum(
                        (getattr(p, field) for p in programmes if getattr(p, field)), 0
                    ),
                )
            # join json
            for field in [
            ...
            for p in programmes[1:]:
                ...
                p.delete()
    print(f"Total programmes to merge {count}")
    print(f"Failed {failed}")


# lot convention programme à basculer
