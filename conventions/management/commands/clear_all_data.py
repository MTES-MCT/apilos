...
class Command(BaseCommand):
    def handle(self, *args, **options):
        if settings.ENVIRONMENT in ["production", "siap-production"]:
            self.stdout.write(
                self.style.ERROR(
                    "This action can't be called in production environment"
                )
            )
            return
        ...
        with transaction.atomic():
            ...
            self.stdout.write("Administration were cleared")
            Bailleur.objects.all().delete()
            self.stdout.write("Bailleur were cleared")
            User.objects.exclude(is_superuser=True).exclude(is_staff=True).delete()
            self.stdout.write("non staff and superuser User were cleared")
