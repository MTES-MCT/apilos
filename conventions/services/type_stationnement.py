...
class ConventionTypeStationnementService(ConventionService):
            ...
            if form_stationnement["uuid"].value():
                ...
                initformset = {
                    **initformset,
                    ...
                    f"form-{idx}-financement": form_stationnement["financement"].value(),
    ...
    def _save_stationnements(self):
        ...
        TypeStationnement.objects.filter(lot_id__in=self.convention.lots.values_list('id', flat=True)).exclude(
            uuid__in=obj_uuids
        ).delete()
        for form_stationnement in self.formset:
            if form_stationnement.cleaned_data["uuid"]:
                ...
                stationnement = TypeStationnement.objects.create(
                    lot=self.convention.lots.get(financement=form_stationnement.cleaned_data["financement"]),
